...
GLuint normalTexOnGPU;
GLuint rhinoTexOnGPU;
GLuint albedoTexOnGPU;
...
struct Scene {
  ...
  void render()
  {
    ...
    for(int i=0; i<lights.size(); ++i) {
      ...
      light.setupCameraForShadowMapping(shadomMapShader, scene_center, scene_radius*1.5);
      light.bindShadowMap();
    ...
    for(int i=0; i<lights.size(); ++i) {
      ...
      mainShader->set(std::string("shadowMap[")+std::to_string(i)+std::string("]"), (int)light.shadowMapTexOnGPU);
      mainShader->set(std::string("mvpLight[")+std::to_string(i)+std::string("]"),light.depthMVP );
    }

    // floor
    mainShader->set("defaultTexture", glm::vec3(0.8, 0.8, 0.9));
    mainShader->set("modelMat", floorMat);
    mainShader->set("normMat", glm::mat3(glm::inverseTranspose(floorMat)));
    mainShader->set("isWooden", false);
    mainShader->set("applyAlbedoTexture", false);

    plane->render();

    // back-wall
    mainShader->set("defaultTexture", glm::vec3(0.29, 0.51, 0.82)); // default value if the texture was not loaded
    mainShader->set("modelMat", planeMat);
    mainShader->set("normMat", glm::mat3(glm::inverseTranspose(planeMat)));
    mainShader->set("isWooden", true);
    mainShader->set("applyAlbedoTexture", true);

    glEnable(GL_TEXTURE_2D);
    glActiveTexture(GL_TEXTURE0+normalTexOnGPU);
    glBindTexture(GL_TEXTURE_2D,normalTex);
    mainShader->set("material.normalTex",(int)normalTexOnGPU);

    glActiveTexture(GL_TEXTURE0+albedoTexOnGPU);
    glBindTexture(GL_TEXTURE_2D,albedoTex);
    mainShader->set("material.albedo",(int)albedoTexOnGPU);

    plane->render();

    // rhino
 
    glActiveTexture(GL_TEXTURE0+rhinoTexOnGPU);
    glBindTexture(GL_TEXTURE_2D,rhinoTex);
    mainShader->set("material.albedo",(int)rhinoTexOnGPU);
    mainShader->set("defaultTexture", glm::vec3(1, 0.71, 0.29));
...
void initScene(const std::string &meshFilename)
{
  ...
  normalTexOnGPU = g_availableTextureSlot;
  ++g_availableTextureSlot;

  albedoTex=loadTextureFromFileToGPU("data/color.png");
  albedoTexOnGPU = g_availableTextureSlot;
  ++g_availableTextureSlot;

  rhinoTex=loadTextureFromFileToGPU("data/rhino_tex.png");
  rhinoTexOnGPU = g_availableTextureSlot;
  ++g_availableTextureSlot; 
